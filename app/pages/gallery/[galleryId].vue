<template>
  <div class="container mx-auto p-4">
    <div v-if="error" class="text-red-500 font-medium text-center p-4">
...
const { data: galleryData, error } = await useFetch(`/api/galleries/${route.params.galleryId}`)
...
watch(galleryData, (newData) => {
  if (newData?.images) {
    images.value = newData.images.map((img, index) => {
